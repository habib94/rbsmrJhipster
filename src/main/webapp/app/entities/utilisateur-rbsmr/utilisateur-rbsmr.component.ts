...
@Component({
    selector: 'jhi-utilisateur-rbsmr',
    templateUrl: './utilisateur-rbsmr.component.html'
})
export class UtilisateurRbsmrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.utilisateurService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<UtilisateurRbsmr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/utilisateur-rbsmr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/utilisateur-rbsmr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: UtilisateurRbsmr) {
        return item.id;
    }
