...
@Component({
    selector: 'jhi-question-rbsmr',
    templateUrl: './question-rbsmr.component.html'
})
export class QuestionRbsmrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.questionService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<QuestionRbsmr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/question-rbsmr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/question-rbsmr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: QuestionRbsmr) {
        return item.id;
    }
