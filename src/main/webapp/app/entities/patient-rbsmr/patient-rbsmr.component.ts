...
@Component({
    selector: 'jhi-patient-rbsmr',
    templateUrl: './patient-rbsmr.component.html'
})
export class PatientRbsmrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.patientService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<PatientRbsmr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/patient-rbsmr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/patient-rbsmr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PatientRbsmr) {
        return item.id;
    }
